...
function exportXLSXGastosDetalle(rows){
  try{
    ...
    // =========================================================
    // ✅ ESTILOS VISUALES (bordes + totales + colores por moneda)
    // =========================================================
    
    // Colores pasteles (puedes ajustar)
    const PALETTE = {
      USD: 'D6FFD6', // verde pastel
      CLP: 'FFD6D6', // rojo pastel
      ARS: 'CFEFFF', // celeste pastel
      BRL: 'FFF5CC', // amarillo pastel
      OTHER: 'F2F2F2', // gris suave para otras monedas
      HEADER: 'EDEDED', // header
      TOTAL: 'D9E8FF'   // totales destacados
    };
    ...
    function getCell(ws, r0, c0){
      const addr = XLSX.utils.encode_cell({ r: r0, c: c0 });
      if (!ws[addr]) ws[addr] = { t:'s', v:'' };
      return ws[addr];
    }
    
    function applyStyle(ws, r0, c0, s){
      const cell = getCell(ws, r0, c0);
      cell.s = cell.s ? { ...cell.s, ...s } : s;
    }
    
    // Determina moneda por nombre de columna (header)
    function monedaFromHeader(h){
      const t = (h || '').toUpperCase();
      if (t.includes('(USD)') || t.includes(' SOLO USD')) return 'USD';
      if (t.includes('(CLP)') || t.includes(' SOLO CLP')) return 'CLP';
      if (t.includes('(ARS)') || t.includes(' SOLO ARS')) return 'ARS';
      if (t.includes('(BRL)') || t.includes(' SOLO BRL')) return 'BRL';
    
      // columnas fijas también cuentan
      if (t.includes('VALOR TERRESTRE (USD)') || t.includes('VALOR HOTEL (USD)') || t.includes('VALOR SEGURO (USD)')) return 'USD';
      if (t.includes('VALOR AÉREO (CLP)') || t.includes('VALOR TERRESTRE (CLP)') || t.includes('VALOR HOTEL (CLP)') || t.includes('VALOR COORDINADOR/A (CLP)')) return 'CLP';
    
      return null;
    }
    
    function fillByMon(mon){
      if (!mon) return PALETTE.OTHER;
      return PALETTE[mon] || PALETTE.OTHER;
    }
    
    function isTotalHeader(h){
      const t = (h || '').toUpperCase();
      return t.startsWith('TOTAL ') || t.includes('TOTAL ACTIVIDADES') || t.includes('TOTAL COMIDAS') || t.includes('TOTAL GASTOS APROB');
    }
    
    // 1) Calcular rango y fila header
    const range = XLSX.utils.decode_range(wsMacro['!ref']); // incluye todo
    const headerRow0 = 2; // aoa: titulo(0), blanco(1), header(2), data desde (3)
    
    // 2) Mapa colIdx -> headerText
    const headerTextByCol = new Map();
    for (let c = range.s.c; c <= range.e.c; c++){
      const cell = getCell(wsMacro, headerRow0, c);
      const text = (cell.v ?? '').toString();
      headerTextByCol.set(c, text);
    }
    
    // 3) Estilo header (fondo gris + bold + borde)
    for (let c = range.s.c; c <= range.e.c; c++){
      applyStyle(wsMacro, headerRow0, c, {
        font: { bold: true },
    ...
    // 4) Estilo body: bordes + moneda por columna + destacar totales
    for (let r = headerRow0 + 1; r <= range.e.r; r++){
      for (let c = range.s.c; c <= range.e.c; c++){
        const h = headerTextByCol.get(c) || '';
        const mon = monedaFromHeader(h);
        const bg = fillByMon(mon);
        const totalCol = isTotalHeader(h);
    
        // Si es columna total: destaque extra (bold + un fondo especial)
        const fillRgb = totalCol ? PALETTE.TOTAL : bg;
    
        const cell = getCell(wsMacro, r, c);
    
        // Si es numérico (o fórmula) lo dejamos como número para que Excel lo trate bien
        // (si tu celda ya trae número, no tocamos el type)
        const isNumericLike = (typeof cell.v === 'number') || !!cell.f;
    
        applyStyle(wsMacro, r, c, {
          font: totalCol ? { bold: true } : undefined,
          fill: { patternType: 'solid', fgColor: { rgb: fillRgb } },
          alignment: { vertical: 'center', horizontal: isNumericLike ? 'right' : 'left', wrapText: true },
          border: BORDER_THIN
        });
    
        // Formato número (puedes ajustar a miles sin decimales)
        if (isNumericLike){
          cell.t = cell.t || 'n';
          cell.z = '#,##0';
        }
    ...
    // 5) Congelar panel (header fijo + columnas base)
    wsMacro['!freeze'] = { xSplit: 7, ySplit: 3 }; 
    // xSplit=7 congela hasta "NOCHES" (col G si tu layout coincide)
    // ySplit=3 congela hasta header (fila 3 en 1-based)
    
    // 6) Altura header un poco mayor (opcional)
    wsMacro['!rows'] = wsMacro['!rows'] || [];
    wsMacro['!rows'][headerRow0] = { hpt: 28 };

...
// 1) Autenticación y guardamos correo
...
async function renderVuelos() {
  ...
  vuelos.forEach(v => {
    ...
    card.innerHTML = `
      ...
      <p><strong>Total Pax:</strong> ${totA+totE} (A:${totA} E:${totE}) – Confirmados: ${confA+confE} (A:${confA} E:${confE})</p>
      <div class="actions">
...
// 7) Guardar/actualizar + escribir en historial
...
// 12) Abrir modal de Grupo (igual al grupo.html)
window.openGroupModal = grupoId => {
  const g = grupos.find(x=>x.id===grupoId);
  if(!g)return alert('Grupo no existe');
  // rellenar…
...
// 13) …y los handlers de grupo (igual a grupos.html)…
...
// 14) Atachar los listeners de grupo en init()

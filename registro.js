...
const campos = [
  ...
  'destino','programa','fechaInicio','duracion','noches','fechaFin','adultos','estudiantes',
  'asistenciaEnViajes','autorizacion','fechaDeViaje','vendedora'
];

let HOTELES_MAP = {};

// Referencias DOM\ nconst elems = {};
...
auth.onAuthStateChanged(user=>{ if(!user) return location.href='login.html'; init(); });

async function init(){
  const ventas = await (await fetch(sheetURL)).json();
  const anos = [...new Set(ventas.map(r=>r.anoViaje))].sort();
  elems.filtroAno.innerHTML = `<option value="">Todos</option>`+anos.map(a=>`<option>${a}</option>`).join('');
...
function buildMaps(ventas){
  ...
  dests.forEach(d=>{ HOTELES_MAP[d] = [...new Set(ventas.filter(r=>r.destino===d).map(r=>r.hotel).filter(Boolean))]; });
}

function setup(ventas){
  elems.filtroAno.onchange = ()=> filterVentas(ventas);
  ['numeroNegocio','nombreGrupo'].forEach(id=> elems[id].onchange = ()=> loadVenta(ventas));
...
function loadVenta(ventas){
  const v = ventas.find(r=> String(r.numeroNegocio)==elems.numeroNegocio.value || r.nombreGrupo===elems.nombreGrupo.value);
  if(!v) return;
  campos.forEach(c=>{ if(!['duracion','noches','fechaFin'].includes(c)) elems[c].value = v[c]||''; });
  onPrograma(); onDestino();
  calcularFin();
  // Preseleccionar hotel original
  const orig = v.hoteles || (v.hotel ? [v.hotel] : []);
  orig.forEach(hot=>{
    const opt = [...elems.hoteles.options].find(o=>o.value===hot);
...
function onDestino(){
  ...
  elems.hoteles.innerHTML = (HOTELES_MAP[d]||[]).map(h=>`<option value="${h}">${h}</option>`).join('');
}

function ajustComp(e){
  const total = Number(elems.cantidadgrupo.value) || 0;
  if(e.target===elems.adultos) elems.estudiantes.value = Math.max(0, total - Number(e.target.value));
  else elems.adultos.value = Math.max(0, total - Number(e.target.value));
}
...
async function guardar(){
  ...
  payload.actualizadoPor = auth.currentUser.email;
  ...
  if(cambios.length){
    ...
    await Promise.all(cambios.map(c=> addDoc(col,{
      numeroNegocio: id, campo:c.campo, anterior:c.anterior, nuevo:c.nuevo,
      modificadoPor: auth.currentUser.email, timestamp: new Date()
    })));
  }
...
async function paintTable(id){
  ...
  const d = snap.data();
  const tbody = elems.tbodyTabla; tbody.innerHTML = '';
  const tr = document.createElement('tr');
  [ d.numeroNegocio, d.nombreGrupo, d.cantidadgrupo, d.colegio, d.curso, d.anoViaje,
    d.destino, d.programa, d.fechaInicio, d.duracion, d.noches, d.fechaFin,
    d.adultos, d.estudiantes, d.asistenciaEnViajes, d.autorizacion, d.fechaDeViaje,
    d.vendedora, (d.hoteles||[]).join(', '), d.actualizadoPor, d.actualizadoEn.toLocaleString()
